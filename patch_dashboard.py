...
# 1. Remove isNotebookOpen state and handleOpenNotebook
content = re.sub(r'const \[isNotebookOpen, setIsNotebookOpen\] = useState\(false\);\n', '', content)

handle_open_notebook_regex = r'  const handleOpenNotebook = \(\) => \{\n    try \{\n      const items = JSON\.parse\(localStorage\.getItem\("vibe_collected_ideas"\) \|\| "\[\]"\);\n      setNotebookItems\(items\);\n    \} catch \{\n      setNotebookItems\(\[\]\);\n    \}\n    setIsNotebookOpen\(true\);\n  \};\n'
content = re.sub(handle_open_notebook_regex, '', content)

# 2. Add useEffect for activeTab === "notebook"
use_effect_notebook = """  useEffect(() => {
...
"""
# Insert it after activeTab definition
if "if (activeTab === \"notebook\") {" not in content:
    content = content.replace("  const [notebookItems, setNotebookItems] = useState<string[]>([]);\n", "  const [notebookItems, setNotebookItems] = useState<string[]>([]);\n\n" + use_effect_notebook)

# 3. Remove Floating Button
floating_btn_regex = r'      \{\/\* Floating Notebook Button \*\/\}.*?<\/button>'
content = re.sub(floating_btn_regex, '', content, flags=re.DOTALL)

# 4. Remove the modal and create the notebook tab content
modal_block_regex = r'      <AnimatePresence>\n        \{isNotebookOpen && \(\n          <motion\.div\n            initial=\{\{ opacity: 0 \}\}\n.*?</AnimatePresence>'
...
# Since we want to place it in the same AnimatePresence block as other tabs, let's look for UnitedEngine or Settings
# Actually, the original code had tabs like `{activeTab === "united-engine" && ...}`.
# I'll just append it right after the united-engine tab block.

united_engine_regex = r'      \{activeTab === "united-engine" && \(\n.*?<\/motion\.div>\n      \)\}'

if "activeTab === \"notebook\"" not in content:
    # First, let's remove the modal block at the bottom
    content = re.sub(modal_block_regex, '', content, flags=re.DOTALL)
    
    # Then insert the notebook tab content after united-engine
    # Let's find united-engine block and append there
    def repl(m):
        return m.group(0) + "\n\n" + notebook_tab_content
    
    content = re.sub(united_engine_regex, repl, content, flags=re.DOTALL)

with open("src/vibe-sandbox/VibeStudentDashboard.tsx", "w") as f:
    f.write(content)
print("Patched VibeStudentDashboard")
